...
class Blockchain:
    def __init__(self):
        ...
        self.shareme_contract = self.w3.eth.contract(
            address=self.w3.to_checksum_address(config.SHAREME_CONTRACT_ADDRESS),
            abi=self.shareme_contract_abi,
        )
        self.avax_usdc_fuji_contract = self.w3.eth.contract(
            address=self.w3.to_checksum_address(config.AVAX_USDC_FUJI_ADDRESS),
            abi=self.avax_usdc_fuji_abi,
        )
    ...
    def get_wallet_created_logs(self, from_block, to_block):
        filter_params = {
            ...
            "topics": [
                "0x5b03bfed1c14a02bdeceb5fa582eb1a5765fc0bc64ca0e6af4c20afc9487f081"
            ],
        }
    ...
    def forward_transfer(self, to_address, token_address, amount):
        ...
        try:
            ...
            gas_estimate = forwarder_contract.functions.executeForwardTransfer(
            ...
            tx = forwarder_contract.functions.executeForwardTransfer(
                walletAddress=to_address,
                token=self.w3.to_checksum_address(token_address),
                amount=amount,
